...
struct timeval presentationTime, unsigned /*durationInMicroseconds*/) {
	...
	{
		if(!strcmp(fSubsession.codecName(), "H264"))	/* h264视频支持 */
		{	
			...
			rtsp_data_info.video_param.pts = (presentationTime.tv_sec + presentationTime.tv_usec/(double)1000000)*90000;	//90KHz乘起来
		...
		else if(!strcmp(fSubsession.codecName(), "MP2T"))	/* TS组播流支持 */
		{
			SLive_RtspDataInfo rtsp_data_info;
	...
	{
		SLive_RtspDataInfo rtsp_data_info;
		if(!strcmp(fSubsession.codecName(), "PCMA"))				//g711a 音频支持
		{
			...
			rtsp_data_info.audio_param.pts = (presentationTime.tv_sec + presentationTime.tv_usec/(double)1000000)*90000; //90KHz乘上来嘛

			rtsp_data_cb_(fReceiveBuffer, frameSize, rtsp_data_info, rtsp_data_cb_user_);

		}
		else if(!strcmp(fSubsession.codecName(), "MPEG4-GENERIC"))	//aac 音频支持
		{
			rtsp_data_info.data_type = LIVE_RTSP_DATA_TYPE_A;
